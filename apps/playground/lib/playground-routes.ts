import {
  DOCUMENT_ROUTES,
  PLATFORM_ROUTES,
  REFERENCE_DATA_ROUTES,
  type DocumentSlug,
  type PlatformSlug,
  type ReferenceDataSlug,
} from './nav';
...
const REFERENCE_DATA_SLUGS = new Set<string>(REFERENCE_DATA_ROUTES.map((route) => route.slug));

export type PlaygroundRoute =
  | { kind: 'home' }
  | { kind: 'document'; slug: DocumentSlug }
  | { kind: 'platform'; slug: PlatformSlug }
  | { kind: 'reference-data'; slug: ReferenceDataSlug };
...
export function resolvePlaygroundRoute(pathname: string): PlaygroundRoute {
  ...
  if (REFERENCE_DATA_SLUGS.has(normalized)) {
    return { kind: 'reference-data', slug: normalized as ReferenceDataSlug };
  }
