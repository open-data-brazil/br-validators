...
|-----------|--------|-----------------|-------|
| **CPF** | Receita Federal | Modulo 11 algorithm; consolidated in RFB technical portals; legal basis RFB | 11 digits, mask `XXX.XXX.XXX-DD`. Two check digits via modulo 11. **Alphanumeric CPF planned for 2026 — no official spec yet.** Validate numeric only until published. |
